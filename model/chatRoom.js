...
async function getChatRoomByRoomId(roomId) {
  const SQL = `SELECT cm.chatRoomId, cm.message, cm.postedByUser, cm.readByUser, cr.userIds, cm.createdAt, cm.updatedAt FROM chatmessages cm inner join chatrooms cr on cm.chatRoomId = cr.id where cr.id = ${roomId} order by cm.createdAt`;
  return new Promise((resolve, reject) => {
    pool.query(SQL, [], (err, result) => {
      if (err) {
        console.log(err);
